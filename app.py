...
BldgType_input = gr.Drowdown(choices =['Single Family Detached', 'Duplex'])
HouseStyle_input = gr.Drowdown(choices =['1 Story', '2 Story'])
OverallQual_input = gr.Number()
YearBuilt_input = gr.Number()
YearRemodAdd_input = gr.Number()
RoofStyle_input = gr.Drowdown(choices =['Gable', 'Hip'])
RoofMaterial_input = gr.Drowdown(choices =['Standard Composite Shingle'])
ExterQual_input = gr.Number()
ExterCond_input = gr.Number()
Foundation_input = gr.Drowdown(choices =['Slab'])
BsmtQual_input = gr.Number()
BsmtCond_input = gr.Number()
BsmtFinSF_input = gr.Number()
TotalBsmtSF_input = gr.Number()
Heating_input = gr.Drowdown(choices =['GasA'])
...
MiscFeature_input = gr.Drowdown(choices =['None', 'Shed'])
