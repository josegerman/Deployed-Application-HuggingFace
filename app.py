...
LotFrontageSF_input = gr.Number(label='Lot Frontage (Squared Feet)',value = 65)
LotAreaSF_input = gr.Number(label='Lot Total Area (Squared Feet)',value=8450)
BldgType_input = gr.Dropdown(label='Build Type',choices =['Single Family Detached', 'Duplex'], value='Single Family Detached')
HouseStyle_input = gr.Dropdown(label='House Style',choices =['1 Story', '2 Story'], value='2 Story')
OverallQual_input = gr.Number(label='Overall House Quality (1 to 10 scale; 10=best)',value=7)
OverallCond_input = gr.Number(label='Overall House Condition (1 to 10 scale; 10=best)',value=5)
YearBuilt_input = gr.Number(label='Year Build',value=2003)
YearRemodAdd_input = gr.Number(label='Year Remodelled (if any)',value=2003)
...
ExterQual_input = gr.Number(label='Exterior Quality (1 to 10 scale; 10=best)',value=4)
ExterCond_input = gr.Number(label='Exterior Condition (1 to 10 scale; 10=best)',value=3)
Foundation_input = gr.Dropdown(label='Foundation Type',choices =['Slab','Poured Concrete'], value='Poured Concrete')
BsmtQual_input = gr.Number(label='Basement Quality (1 to 10 scale; 10=best)',value=4)
BsmtCond_input = gr.Number(label='Basement Condition (1 to 10 scale; 10=best)',value=3)
BsmtFinSF_input = gr.Number(label='Basement Finished Area (Squared Feed)',value=706)
TotalBsmtSF_input = gr.Number(label='Total Basement Area (Squared Feet)',value=856)
...
CentralAir_input = gr.Dropdown(label='Central Air Conditioning',choices =['Yes','No'], value='Yes')
...
GrLivArea_input = gr.Number(label='Great Living Area (Squared Feet)',value=1710)
GarageType_input = gr.Dropdown(label='Garage Type',choices =['Attached', 'Detached'], value='Attached')
GarageYrBlt_input = gr.Number(label='Year Garage Built',value=2003)
GarageCars_input = gr.Number(label='Garage Size (Number of Cars)',value=2)
GarageSF_input = gr.Number(label='Garage Area (Squared Feet)',value=548)
GarageQual_input = gr.Number(label='Garage Quality (1 to 10 scale; 10=best)',value=3)
WoodDeckSF_input = gr.Number(label='Wood Deck Area (Squared Feet)',value=0)
OpenPorchSF_input = gr.Number(label='Open Porch Area (Squared Feet)',value=61)
